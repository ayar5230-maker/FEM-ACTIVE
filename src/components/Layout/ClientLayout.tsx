...
const navItems = [
  { to: '/client', label: 'Accueil', icon: '⌂', end: true },
  { to: '/client/workouts', label: 'Entraînements', icon: '◎' },
  { to: '/client/nutrition', label: 'Nutrition', icon: '◌' },
  { to: '/client/checkin', label: 'Check-in', icon: '◍' },
  { to: '/client/messages', label: 'Messages', icon: '◈', hasUnread: true },
]

export function ClientLayout({ profile }: ClientLayoutProps) {
  ...
  return (
    <div className="flex min-h-screen bg-brand-lavender/20">
      {/* Sidebar (desktop) */}
      <aside className="hidden md:flex flex-col w-64 min-h-screen bg-brand-deep text-white fixed left-0 top-0">
        <div className="px-6 py-7 border-b border-white/10">
          ...
          <p className="font-body text-xs text-brand-lavender/60 mt-0.5">Mon Espace</p>
        </div>
        ...
        <div className="px-4 py-4 border-t border-white/10">
          <div className="flex items-center gap-3 mb-3">
            ...
            <div className="min-w-0">
              <p className="font-body text-sm text-white truncate">{profile.full_name ?? 'Cliente'}</p>
              {profile.forfait && (
                <p className="font-body text-xs text-brand-violet/80 truncate">{profile.forfait}</p>
              )}
            </div>
          </div>
          <button
